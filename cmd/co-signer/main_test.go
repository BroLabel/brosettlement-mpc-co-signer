...
func TestVerifyMPC2of3RejectsPreviousMPCorePin(t *testing.T) {
	...
	if !strings.Contains(string(output), "mpc-core must resolve exactly v0.4.2") {
		t.Fatalf("output = %s, want rejection of v0.4.0", output)
	}
}
...
func TestDKGProvisioningAdmissionHintDependsOnlyOnPreParams(t *testing.T) {
	tests := []struct {
		name           string
		preparamsReady bool
		want           bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := dkgProvisioningAdmissionHint(preparams)
			if got != tt.want {
