...
func probePlatformPublishCapability(directory string) error {
	return probePublishCapability(realPublishOperations{}, readArtifactFile, directory, rand.Reader)
}

func publishArtifactFile(finalPath string, finalBytes []byte) error {
	return publishDurably(realPublishOperations{}, finalPath, finalBytes)
}

type realPublishOperations struct{}

func (realPublishOperations) CreateTemp(directory, pattern string) (publishTempFile, error) {
	return os.CreateTemp(directory, pattern)
}

func (realPublishOperations) Remove(path string) error { return os.Remove(path) }

func (realPublishOperations) RenameNoReplace(oldPath, newPath string) error {
...
func (realPublishOperations) SyncDirectory(directory string) error {
