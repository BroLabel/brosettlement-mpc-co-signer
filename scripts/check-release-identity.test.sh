...
grep -Fx 'version=2.1.0' <<<"$main_output" >/dev/null
...
release_output=$(run_check refs/tags/v2.1.0)
grep -Fx 'tags=ghcr.io/brolabel/brosettlement-mpc-co-signer:v2.1.0' <<<"$release_output" >/dev/null
